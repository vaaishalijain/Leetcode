"""
    ...
    Q. Given a list of non-overlapping axis-aligned rectangles rects, write a function pick which randomly and
...
class Solution:
    ...
    def __init__(self, rects: List[List[int]]):
        p=[(x2-x1+1)*(y2-y1+1) for x1,y1,x2,y2 in rects]
        print(p)
        self.w=[i/sum(p) for i in accumulate(p)]
        self.rects=rects

    def pick(self) -> List[int]:
        cur=bisect.bisect(self.w,random.random())
        x1,y1,x2,y2 = self.rects[cur]
