"""
    ...
    Q. An image is represented by a 2-D array of integers, each integer representing the pixel value of the image
      (from 0 to 65535).
...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, newColor: int) -> List[List[int]]:
        if newColor == image[sr][sc]:
            return image
        q, prev=[(sr, sc)], image[sr][sc]
        while q:
            r, c = q.pop(0)
            if 0 <= r < len(image) and 0 <= c < len(image[0]) and image[r][c] == prev:
                image[r][c] = newColor
                q.append((r+1, c))
                q.append((r-1, c))
                q.append((r, c+1))
