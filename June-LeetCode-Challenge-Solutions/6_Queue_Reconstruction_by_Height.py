"""
    ...
    Q. Suppose you have a random list of people standing in a queue. Each person is described by a pair of integers
...
class Solution:
    def reconstructQueue(self, people: List[List[int]]) -> List[List[int]]:
        ans=[]
        for i,j in sorted(people, key=lambda val: (-val[0], val[1])):
            ans.insert(j, [i,j])
        return ans