"""
    ...
    Q. Write a function to check whether an input string is a valid IPv4 address or IPv6 address or neither.
        ...
        in the address to upper-case ones, so 2001:db8:85a3:0:0:8A2E:0370:7334 is also a valid IPv6 address(Omit
        ...
        def validIPv6():
            ...
            for each in arr:
                if not each or len(each) > 4:
                    return False
                if not re.match('[0-9a-fA-F]', each):
                    return False
                try:
                    int(each, 16)
                except ValueError:
